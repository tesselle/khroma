...
```{r hook, comment="", results="asis", echo=FALSE}
...
All these colour schemes are implemented for use with R `graphics` or [`ggplot2`](https://github.com/tidyverse/ggplot2).
...
If [`crayon`](https://github.com/r-lib/crayon) is installed, colours will be nicely printed in the console.
```{r crayon}
bright(7)
```

You can disable this feature by setting the `khroma.crayon` option to `FALSE`.
```{r no-crayon}
